...
class FileCoder(object):
    ...
    def encode(self, values):
        ...
        cw = csv.writer(os, delimiter=self._delimiter,quotechar='"', quoting=csv.QUOTE_MINIMAL)
...
class PrepareFieldTypes(beam.DoFn):
    ...
    def __init__(self, encoding='utf-8', time_format='%Y-%m-%d %H:%M:%S %Z'):
        ...
        # Add here additional time format to use in case the default one will not work
        ...
        for k, v in element.items():
            ...
            try:
                if not v:
                    ...
                    try:
                        v = self._tm.mktime(self._tm.strptime(
                                        v, self._time_format
                                    ))
                    except (ValueError, TypeError), e:
                        logging.warn('Cannot convert type %s for element %s: %s. Returnin default value.' % (ftype, v, e))
                        v = self._return_default_value(ftype)

                elif ftype == 'TIMESTAMP':
                    for fmt in (self._time_format):
                        try:
                            v = int(self._tm.mktime(self._tm.strptime(
                                v, fmt
                            )))
                        except ValueError, e:
                            pass
                    ...
                    if not isinstance(v, int):
                        logging.warn('Cannot convert date %s. Error: %s' % (v, e))
                        v = self._return_default_value(ftype)
                else:
                    logging.warn('Unknown field type %s' % ftype)
                    v = self._return_default_value(ftype)
            except (TypeError, ValueError), e:
                logging.warn('Cannot convert type %s for element %s: %s. Returning default value.' % (ftype, v, e))
                v = self._return_default_value(ftype)
            element[k] = v
        return [element]
...
def _get_bq_schema(fields):
    ...
    for k, v in fields.items():
        bq_fields.append(TableFieldSchema(
            name=k, type=v, description='Field %s' % k
        ))        
    bq_fields.append(TableFieldSchema(
        name='_RAWTIMESTAMP', type='TIMESTAMP', description='Injected timestamp'
    ))
...
def run(argv=None):
    ...
    parser.add_argument(
        '--input-bucket', dest='input_bucket', required=True, default='data-daimlr',
        help="Input files path in gs ('bucket_name')."
    )

    parser.add_argument(
        '--input-path', dest='input_path', required=True, default='RawData',
        help="Input files path in gs ('path')."
    )    

    parser.add_argument(
        '--input-files', dest='input_files', required=True, 
        help="File name to be imported."
    )

    parser.add_argument(
        '--bq-dataset', dest='bq_dataset', required=True, default='rawdata',
    ...
    for input_file in known_args.input_files.split(','):
        ...
        logging.info('Retreiving information for table %s' % (table_name))

        try:
            table = _fetch_table(table_name)
        except InvalidArgument, e:
            raise SystemExit('Error getting Table information for table [%s] with error: %s' % (table_name, e))
        if not table:
            raise SystemExit('no table found') 

        fields = json.loads(
            table['columns'].decode('utf-8'), object_pairs_hook=OrderedDict
        )       
        gs_path = os.path.join(known_args.input_bucket,*[known_args.input_path, input_file])
        ...
        | 'Write to BigQuery - ' + input_file >> beam.io.Write(beam.io.BigQuerySink(
