...
class BaseWrapper(object):
    ...
    def _get_query_results(self, project_id, job_id,
                         page_token=None, max_results=100000):
        raise NotImplementedError
    ...
    def count(self, query):
        with self.connection.cursor() as cursor:
            print("Estimating size for query")
    ...
    def read(self, query, batch=READ_BATCH):
        print('Ejecuta read desde wrapper en class BaseWrapper')
        """
        ...
        """
        with self.connection.cursor() as cursor:
            print("Executing Read query - "+str(datetime.datetime.now()))
